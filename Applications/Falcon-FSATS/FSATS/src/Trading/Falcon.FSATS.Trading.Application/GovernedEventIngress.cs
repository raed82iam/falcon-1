...
public sealed class GovernedApplicationEventIngress
{
    ...
    public static string BrokerAccountScope(string brokerId, string brokerAccountId, string environment)
        => $"BROKER_ACCOUNT|{Part(brokerId, upper: true)}|{Part(brokerAccountId)}|{Part(environment, upper: true)}";

    public static string ExecutionRouteScope(string brokerId, string brokerAccountId, string environment, string executionRouteId)
        => $"EXECUTION_ROUTE|{Part(brokerId, upper: true)}|{Part(brokerAccountId)}|{Part(environment, upper: true)}|{Part(executionRouteId)}";
    ...
    private static string? Validate(GovernedApplicationEventEnvelope? envelope, DateTimeOffset now)
    {
        ...
        if (!StringComparer.Ordinal.Equals(envelope.SubscriberApplicationId, TradingManifest.Current.ApplicationId)) return "EVENT_SUBSCRIBER_MISMATCH";
    ...
    private static string CanonicalizeScope(string scope)
    {
        ...
        if (parts.Length == 4 && parts[0] == "BROKER_ACCOUNT") return BrokerAccountScope(Decode(parts[1]), Decode(parts[2]), Decode(parts[3]));
        if (parts.Length == 5 && parts[0] == "EXECUTION_ROUTE") return ExecutionRouteScope(Decode(parts[1]), Decode(parts[2]), Decode(parts[3]), Decode(parts[4]));

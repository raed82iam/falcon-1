...
internal static class Program
{
    ...
    private static int Main()
    {
        try
        {
            ...
            var wrongKindDecision = Evaluate(wrongKind, registry);
            Check(!wrongKindDecision.Accepted && wrongKindDecision.Reason == AiKillControlReason.TargetNotFound, "target-kind mismatch was accepted");
            ...
            var fsaAuthority = AuthorityRequestFor(fsaRequest);
            var fsaDecision = AiKillControlRuntime.Evaluate(fsaRequest, registry, fsaAuthority, AuthorityContextFor(fsaRequest, fsaAuthority), Now);
            Check(!fsaDecision.Accepted && fsaDecision.Reason == AiKillControlReason.AiActorProhibited, "FSA invoked its own Kill Control Plane");
            ...
            var stale = registry.Select(item => item.TargetId == "ai:app-alpha:msa" ? item with { Expiry = Now } : item).ToArray();
            Check(Evaluate(target, stale).Reason == AiKillControlReason.InvalidRegistry, "stale root created an orphaned hierarchy instead of failing closed");

            var definedGroup = Request("kill:group", "owner:governed", AiKillIngress.ExternalOwner, AiEmergencyAction.Isolate, AiTargetKind.DefinedGroup, "ai-group:alpha");
            var groupDecision = Evaluate(definedGroup, registry);
            ...
            var restrict = Request("kill:restrict", "owner:governed", AiKillIngress.WebOwner, AiEmergencyAction.Restrict, AiTargetKind.Component, "ai:app-beta:component");
            var restrictDecision = Evaluate(restrict, registry);
            Check(restrictDecision.Accepted && !restrictDecision.StopRequired && !restrictDecision.IsolationRequired && restrictDecision.AuthorityRevocationRequired, "Restrict action semantics drifted");
            var suspend = Request("kill:suspend", "owner:governed", AiKillIngress.WebOwner, AiEmergencyAction.Suspend, AiTargetKind.Component, "ai:app-beta:component");
            var suspendDecision = Evaluate(suspend, registry);
            Check(suspendDecision.Accepted && suspendDecision.SuspensionRequired && !suspendDecision.StopRequired, "Suspend semantics drifted");

            var enforcer = new AiKillControlAuthorityEnforcer();
            var killedRequest = ExecutionRequest("exec:killed", "ai:app-alpha:csa:one", "EXECUTE", "resource:alpha", "application:alpha");
            var killedResult = enforcer.Evaluate(killedRequest, ExecutionContext(killedRequest), new[] { targetDecision });
            ...
            var restartedRequest = killedRequest with { RequestId = "exec:restarted", Correlation = "correlation:exec:restarted" };
            var restartedResult = enforcer.Evaluate(restartedRequest, ExecutionContext(restartedRequest), new[] { targetDecision });
            Check(restartedResult.Decision == AuthorityDecision.Deny, "restart restored killed AI authority");

            var safeCoreRequest = ExecutionRequest("exec:safe-core", "owner-control:primary", "CONTROL", "foundation:safe-core", "foundation:safe-core");
            var safeCoreResult = enforcer.Evaluate(safeCoreRequest, ExecutionContext(safeCoreRequest), new[] { globalDecision });
            Check(safeCoreResult.Decision == AuthorityDecision.Allow, "global AI Kill stopped non-AI Safe Core authority");
            ...
            var noControls = enforcer.Evaluate(safeCoreRequest, ExecutionContext(safeCoreRequest), Array.Empty<AiKillControlDecision>());
            Check(noControls.Decision == AuthorityDecision.Allow, "empty Kill evidence changed baseline authority");
            Check(enforcer.Evaluate(safeCoreRequest, ExecutionContext(safeCoreRequest), null).Reason == AiKillAuthorityReason.EvidenceUnavailable, "missing Kill evidence did not fail closed");
            ...
            var webDecision = Evaluate(Request("kill:web-ingress", "owner:governed", AiKillIngress.WebOwner, AiEmergencyAction.Kill, AiTargetKind.Component, "ai:app-beta:component"), registry);
            var externalDecision = Evaluate(Request("kill:external-ingress", "owner:governed", AiKillIngress.ExternalOwner, AiEmergencyAction.Kill, AiTargetKind.Component, "ai:app-beta:component"), registry);
            Check(webDecision.Accepted && externalDecision.Accepted && webDecision.TargetId == externalDecision.TargetId, "dual ingress did not converge on one control plane semantics");
            Check(webDecision.Ingress != externalDecision.Ingress, "independent ingress identity was lost");
            ...
            Check(_checks == 40, $"Unexpected check count: {_checks}, expected 40.");
            Console.WriteLine("STAGE13_WP01_AI_KILL_CONTROL_VERIFIER = PASS");
            Console.WriteLine("CHECKS = 40/40");
    ...
    private static AuthorityRequest AuthorityRequestFor(AiKillRequest request) => new(
        "authority:" + request.RequestId, request.ActorIdentity, AiKillControlRuntime.ToAuthorityAction(request.Action), request.TargetId,
        AiKillControlPlaneContract.Purpose, request.TargetId, "EMERGENCY_CONTROL", "TRUSTED", "FIT", request.Correlation,
        request.RequestTime, request.Expiry);
    ...
    private static AuthorityEvaluationContext AuthorityContextFor(AiKillRequest request, AuthorityRequest authority, string? allowedActionOverride = null) => new(
        new AuthorityPolicy("policy:" + request.RequestId, "1.0", "owner:governed", Now.AddHours(-1), Now.AddHours(2),
        ...
        Now, "authority-evidence:" + request.RequestId);

    private static AuthorityRequest ExecutionRequest(string id, string actor, string action, string resource, string scope) =>
        new(id, actor, action, resource, "EXECUTION", scope, "LIVE", "TRUSTED", "FIT", "correlation:" + id, Now.AddMinutes(-1), Now.AddDays(2));
    ...
    private static AuthorityEvaluationContext ExecutionContext(AuthorityRequest request, DateTimeOffset? observation = null)
    {
        ...
        return new AuthorityEvaluationContext(
            ...
            time, "authority-evidence:" + request.RequestId);
    }

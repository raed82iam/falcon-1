...
static ApplicationSubjectEvidence BuildApplicationSubject(ApplicationManifest manifest, AdmissionRequest request, AdmissionDecision decision, string evidenceReference)
{
    ...
    if (string.IsNullOrWhiteSpace(decision.EvidenceId))
    {
        throw new InvalidOperationException("WP06_ADMISSION_DECISION_EVIDENCE_ID_MISSING");
    }

    return new ApplicationSubjectEvidence
    {
        ...
        EvidenceReference = decision.EvidenceId,
        AdmissionRequest = request,
        AdmissionDecision = decision,
        Manifest = manifest,
        ManifestDigest = manifest.ComputeDigest(),
        AdmissionEvidenceIdentity = decision.EvidenceId
    };
}
...
static ServiceCatalogEntry CreateServiceCatalogEntry(DateTimeOffset observationTime, ICollection<string> failures)
{
    ...
    if (!result.Success || result.Decision != ServiceRegistrationDecision.Registered)
    {
        throw new InvalidOperationException(
            $"WP06_SERVICE_REGISTRATION_REJECTED:{result.ReasonCode}");
    }

    var lookup = catalog.Lookup(serviceIdentity, serviceVersion);
    if (lookup is null || lookup.Entry is null)
    {
        throw new InvalidOperationException(
            "WP06_SERVICE_LOOKUP_MISSING_AFTER_SUCCESSFUL_REGISTRATION");
    }

    Expect(failures, "service-registration", true, result.ReasonCode);
    Expect(failures, "service-lookup", true);
    return lookup.Entry;
}
...
try
{
    Console.WriteLine("WP06-CHECKPOINT-01: starting deterministic fixture construction");
    Console.Out.Flush();
    var observationTime = DateTimeOffset.Parse("2026-08-01T00:20:00+03:00", CultureInfo.InvariantCulture);
    
    Console.WriteLine("WP06-CHECKPOINT-02: registering governed service");
    Console.Out.Flush();
    var serviceEntry = CreateServiceCatalogEntry(observationTime, failures);
    
    Console.WriteLine("WP06-CHECKPOINT-03: creating application manifests");
    Console.Out.Flush();
    ...
    Console.WriteLine("WP06-CHECKPOINT-04: constructing admission baseline");
    Console.Out.Flush();
    ...
    var admissionRequest = CreateAdmissionRequest(applicationManifest, observationTime) with
    {
        AdmissionId = $"admission:{applicationManifest.ApplicationIdentity}:{applicationManifest.ApplicationVersion}"
    };
    Console.WriteLine("WP06-CHECKPOINT-05: evaluating primary application admission");
    Console.Out.Flush();
    var admissionDecision = admissionControl.Evaluate(admissionRequest);
    if (admissionDecision is null)
    {
        throw new InvalidOperationException("WP06_PRIMARY_ADMISSION_RETURNED_NULL");
    }
    Expect(
        failures,
        "application-admission-evaluate",
        string.Equals(admissionDecision.Decision, "ADMITTED", StringComparison.Ordinal),
        admissionDecision.ReasonCode);
    
    var graphAdmissionRequest = CreateAdmissionRequest(applicationServiceManifest, observationTime) with
    {
        AdmissionId = $"admission:{applicationServiceManifest.ApplicationIdentity}:{applicationServiceManifest.ApplicationVersion}"
    };
    Console.WriteLine("WP06-CHECKPOINT-06: evaluating graph application admission");
    Console.Out.Flush();
    var graphAdmissionDecision = admissionControl.Evaluate(graphAdmissionRequest);
    if (graphAdmissionDecision is null)
    {
        throw new InvalidOperationException("WP06_GRAPH_ADMISSION_RETURNED_NULL");
    }
    Expect(
        failures,
        "graph-application-admission-evaluate",
        string.Equals(graphAdmissionDecision.Decision, "ADMITTED", StringComparison.Ordinal),
        graphAdmissionDecision.ReasonCode);
    
    Console.WriteLine("WP06-CHECKPOINT-07: constructing governed dependency graph");
    Console.Out.Flush();
    ...
    Console.WriteLine("WP06-CHECKPOINT-08: validating governed dependency graph");
    Console.Out.Flush();
    var validResult = validator.Validate(validRequest);
    if (!validResult.Success)
    {
        throw new InvalidOperationException(
            $"WP06_DEPENDENCY_GRAPH_REJECTED:{validResult.ReasonCode}");
    }
    ...
    Expect(failures, "graph-determinism-order", string.Equals(validResult.CanonicalActivationOrderText, repeatedResult.CanonicalActivationOrderText, StringComparison.Ordinal), validResult.CanonicalActivationOrderText);
        ...
        const string expectedGoldenDigest = "D06C6EDE16D2A55F4FBA36B965C5EECA0A98CE5AE11CE711ABCB4E8FECFF992E";
        const int expectedGoldenByteLength = 4962;
        ...
        // Stage 3 WP-06 canonical end-to-end seam:
        // actual admission decision -> actual service registration -> actual dependency result
        // -> canonical bootstrap gate -> lifecycle registration.
        var wp06Subject = new BootstrapSubjectAdmissionEvidence
        {
            SubjectId = graphAdmissionRequest.Identity,
            SubjectVersion = graphAdmissionRequest.Version,
            SubjectKind = BootstrapSubjectKind.Plugin,
            ArtifactIdentity = $"artifact:{graphAdmissionRequest.Identity}:{graphAdmissionRequest.Version}",
            ArtifactDigest = Sha256(applicationServiceManifest.PackageContentOrIntegrityInput),
            ManifestIdentity = $"manifest:{graphAdmissionRequest.Identity}:{graphAdmissionRequest.Version}",
            ManifestDigest = graphAdmissionRequest.ManifestDigest,
            AdmissionDecisionId = graphAdmissionDecision.AdmissionId,
            AdmissionState = graphAdmissionDecision.Decision,
            RegistrationEvidenceId = $"not-applicable:{graphAdmissionRequest.Identity}:{graphAdmissionRequest.Version}",
            RegistrationState = "NOT_APPLICABLE",
            EvidenceAuthority = "FALCON-STAGE3-WP05-EVIDENCE",
            EffectiveTime = observationTime.AddMinutes(-5),
            Expiry = observationTime.AddHours(2)
        };
    
        var wp06Context = new BootstrapExecutionContextRecord(
            $"context:{wp06Subject.SubjectId}:{wp06Subject.SubjectVersion}",
            "1.0",
            "FALCON-STAGE3-WP05-AUTHORITY",
            "ENV-STAGE3-WP05-ISOLATED",
            "STAGE3-WP05-BOOTSTRAP-AND-LIFECYCLE",
            "EXTERNAL-BOOTSTRAP-CONTROL",
            $"context-evidence:{wp06Subject.SubjectId}",
            "DEFINED",
            "NO-PRODUCTION-NO-FINANCIAL-NO-EXTERNAL-CONNECTIVITY",
            observationTime.AddHours(-1),
            observationTime.AddHours(2));
    
        var wp06Provenance = new BootstrapEvidenceProvenanceRecord(
            $"provenance:{wp06Subject.SubjectId}:{wp06Subject.SubjectVersion}",
            "1.0",
            $"source-record:{wp06Subject.SubjectId}:{wp06Subject.SubjectVersion}",
            wp06Subject.ArtifactDigest,
            wp06Context.SourceIdentity,
            "FALCON-STAGE3-WP05-PROVENANCE-AUTHORITY",
            $"provenance-evidence:{wp06Subject.SubjectId}",
            "PROVEN",
            wp06Subject.ArtifactIdentity,
            observationTime.AddHours(-1),
            observationTime.AddHours(2));
    
        var wp06TimeProvider = new TimeProviderRecord(
        ...
        var wp06DependencyEvidence = new DependencyActivationEvidence
        {
            SubjectId = wp06Subject.SubjectId,
            SubjectVersion = wp06Subject.SubjectVersion,
            GraphId = "stage3-wp04-golden-graph",
            GraphVersion = "1.0",
            GraphDigest = validResult.GraphDigest,
            DependencyValidationState = validResult.GraphDecision == "DEPENDENCY_GRAPH_VALIDATED"
                ? "VALIDATED"
                : validResult.GraphDecision,
            ActivationOrderState = validResult.ActivationOrderDecision == "ACTIVATION_ORDER_VALIDATED"
                ? "VALIDATED"
                : validResult.ActivationOrderDecision,
            SubjectActivationIndex = 4,
            EvidenceReference = $"dependency-evidence:{wp06Subject.SubjectId}",
            EffectiveTime = observationTime.AddMinutes(-5),
            Expiry = observationTime.AddHours(2)
        };
    
        var wp06BootstrapRequest = new BootstrapValidationRequest
        {
            RequestId = $"bootstrap-request:{wp06Subject.SubjectId}:{wp06Subject.SubjectVersion}",
            Subject = wp06Subject,
            Context = wp06Context,
            Provenance = wp06Provenance,
            TimeProvider = wp06TimeProvider,
            DependencyEvidence = wp06DependencyEvidence,
            ObservationTime = observationTime
        };
    
        var wp06Bootstrap = BootstrapContextGate.Validate(wp06BootstrapRequest);
        var wp06BootstrapRepeat = BootstrapContextGate.Validate(wp06BootstrapRequest);
        Expect(failures, "wp06-bootstrap-accepted", wp06Bootstrap.Accepted, wp06Bootstrap.ReasonCode);
        Expect(
            failures,
            "wp06-bootstrap-deterministic",
            string.Equals(wp06Bootstrap.DecisionIdentity, wp06BootstrapRepeat.DecisionIdentity, StringComparison.Ordinal),
            $"{wp06Bootstrap.DecisionIdentity} vs {wp06BootstrapRepeat.DecisionIdentity}");
    
        var wp06BadDependency = BootstrapContextGate.Validate(
            wp06BootstrapRequest with
            {
                DependencyEvidence = wp06DependencyEvidence with
                {
                    GraphDigest = new string('0', 64)
                }
            });
        Expect(
            failures,
            "wp06-mutated-graph-digest-fails-closed",
            !wp06BadDependency.Accepted &&
            string.Equals(wp06BadDependency.ReasonCode, "DEPENDENCY_EVIDENCE_BINDING_MISMATCH", StringComparison.Ordinal),
            wp06BadDependency.ReasonCode);
    
        var wp06Lifecycle = new LifecycleControlService(LifecycleStateModel.CreateCanonical());
        var wp06Registration = wp06Lifecycle.Register(
            wp06BootstrapRequest,
            $"wp06-e2e:{graphAdmissionDecision.EvidenceId}:{serviceEntry.Registration.RegistrationEvidenceReference}:{validResult.DecisionIdentity}");
    
        Expect(
            failures,
            "wp06-lifecycle-registration",
            wp06Registration.Registration.Accepted,
            wp06Registration.Registration.ReasonCode);
    
        var wp06EvidenceProjection = string.Join(
            "\n",
            graphAdmissionDecision.AdmissionId,
            graphAdmissionDecision.EvidenceId,
            serviceEntry.Registration.RegistrationId,
            serviceEntry.Registration.RegistrationEvidenceReference,
            serviceEntry.Registration.ManifestDigest,
            validResult.GraphDigest,
            validResult.ActivationOrderDigest,
            validResult.DecisionIdentity,
            wp06Bootstrap.DecisionIdentity,
            wp06Registration.Registration.Snapshot?.SubjectId ?? string.Empty,
            wp06Registration.Registration.Snapshot?.State.ToString() ?? string.Empty,
            wp06Registration.Registration.Snapshot?.StateVersion.ToString(CultureInfo.InvariantCulture) ?? string.Empty);
    
        var wp06EvidenceDigest = Sha256(wp06EvidenceProjection);
        var wp06EvidenceDigestRepeat = Sha256(wp06EvidenceProjection);
    
        Expect(
            failures,
            "wp06-end-to-end-evidence-deterministic",
            string.Equals(wp06EvidenceDigest, wp06EvidenceDigestRepeat, StringComparison.Ordinal),
            $"{wp06EvidenceDigest} vs {wp06EvidenceDigestRepeat}");
    
        Console.WriteLine($"WP-06 End-to-End Evidence SHA-256: {wp06EvidenceDigest}");
        ...
        var applicationEvidenceMismatchSubject =
            BuildApplicationSubject(
                applicationEvidenceMismatchManifest,
                applicationEvidenceMismatchRequest,
                applicationEvidenceMismatchDecision,
                "evidence/admission/mismatch") with
            {
                AdmissionEvidenceIdentity = "evidence/admission/different"
            };
    ...
    if (failures.Count > 0)
    {
        Console.Error.WriteLine("Stage 3 WP-06: FAIL");
    ...
    Console.WriteLine("Stage 3 WP-06: PASS");
    Console.WriteLine("DEPENDENCY_GRAPH_VALIDATED");
    Console.WriteLine("ACTIVATION_ORDER_VALIDATED");
    Console.WriteLine("END_TO_END_PLUGIN_ADMISSION_BOOTSTRAP_LIFECYCLE_VALIDATED");
    Console.WriteLine("WP06_DETERMINISTIC_EVIDENCE_VALIDATED");
    return 0;
}
catch (Exception ex)
{
    Console.WriteLine("WP06-UNHANDLED-EXCEPTION");

...
internal static class ProgramV2
{
    ...
    private static int Main()
    {
        ...
        Run("fcr_census_and_disposition_valid", () => ValidateFcrArtifacts(root, censusPath, dispositionPath));
        ...
        Run("deterministic_integrated_closure_identity", () => DeterministicIntegratedClosureIdentity(root, manifestPath, censusPath, dispositionPath));
        Run("no_stage7_or_authority_claim", NoStage7OrAuthorityClaim);

        Console.WriteLine();
        Console.WriteLine($"STAGE 6 WP-10 VERIFIER V2: {_passed}/{_passed + _failed} PASS");
    ...
    private static void ValidateCanonicalEvidenceForRow(string root, Dictionary<string, string> row)
    {
        ...
        Require(row["closure_evidence_kind"] == "CANONICAL_CLOSURE_RECORD", $"False historical classification for {wp}.");
        ...
        Require(IsSafeRelativePath(relative), $"Unsafe closure locator for {wp}.");
        var full = Path.Combine(root, relative.Replace('/', Path.DirectorySeparatorChar));
        Require(File.Exists(full), $"Canonical closure record missing for {wp}: {relative}");
        Require(GitObjectExists(root, "HEAD:" + relative), $"Canonical closure record not tracked at validation HEAD for {wp}: {relative}");
        ...
        var canonicalDigest = Sha256GitBlob(root, "HEAD:" + relative);
        Require(StringComparer.Ordinal.Equals(canonicalDigest, row["closure_evidence_sha256"]), $"Canonical closure SHA-256 mismatch for {wp}: {relative}");

        var closureText = GitBlobText(root, "HEAD:" + relative);
        Require(closureText.Contains(wp, StringComparison.Ordinal), $"Closure evidence substitution detected for {wp}.");
        Require(closureText.Contains("ACCEPTED_AND_CLOSED", StringComparison.Ordinal), $"Closure record for {wp} lacks accepted-and-closed evidence.");
    }
    ...
    private static void ValidateManifestRows(IReadOnlyList<Dictionary<string, string>> rows)
    {
        ...
        Require(expected.SequenceEqual(rows.Select(r => r["work_package"]), StringComparer.Ordinal), "Manifest WP set/order mismatch.");
        ...
        Require(rows.All(r => r["closure_evidence_kind"] == "CANONICAL_CLOSURE_RECORD"), "Invalid closure evidence kind.");
        ...
        Require(rows.All(r => !r["application_compatibility_disposition"].Contains("CLOSED", StringComparison.OrdinalIgnoreCase)), "Application compatibility must never encode Application closure.");

        foreach (var row in rows)
        {
            var wp = row["work_package"];
            Require(ExpectedScopeByWp.TryGetValue(wp, out var scope) && StringComparer.Ordinal.Equals(scope, row["accepted_scope_label"]), $"Functional-chain scope mismatch for {wp}.");
    ...
    private static void ValidateFcrArtifacts(string root, string censusPath, string dispositionPath)
    {
        ...
        var relativeCensus = NormalizeRelative(root, censusPath);
        ValidateFcrRows(census.Rows, disposition.Rows, Sha256GitBlob(root, "HEAD:" + relativeCensus));
    }
    ...
    private static void ValidateFcrRows(IReadOnlyList<Dictionary<string, string>> census, IReadOnlyList<Dictionary<string, string>> dispositions, string censusSha)
    {
        ...
        var cv = census.Select(r => r["census_version"]).Distinct(StringComparer.Ordinal).ToArray();
        var sv = dispositions.Select(r => r["snapshot_version"]).Distinct(StringComparer.Ordinal).ToArray();
        var cvi = 0;
        var svi = 0;
        Require(cv.Length == 1 && int.TryParse(cv[0], out cvi) && cvi > 0, "Invalid census version.");
        Require(sv.Length == 1 && int.TryParse(sv[0], out svi) && svi > 0, "Invalid snapshot version.");
        Require(cvi == svi, "Census and disposition snapshot versions must match.");

        var capture = census.Select(r => r["captured_at_utc"]).Distinct(StringComparer.Ordinal).ToArray();
        var capturedAt = default(DateTimeOffset);
        Require(capture.Length == 1 && TryParseUtc(capture[0], out capturedAt), "Invalid census capture time.");
        ...
        foreach (var pair in relevant)
        {
            ...
            {
                Require(c["waiting_on"] == "APPLICATION", $"Application-owned trigger must hand off to APPLICATION: issue {pair.Key}");
                Require(c["status"] is "FOUNDATION_IMPLEMENTED" or "APPLICATION_VERIFIED", $"Application-owned trigger requires post-Foundation lifecycle state: issue {pair.Key}");
                Require(!c["status"].Contains("CLOSED", StringComparison.OrdinalIgnoreCase), $"Application-owned trigger must not encode closure: issue {pair.Key}");
                Require(d["disposition_basis"].Contains("FCR remains OPEN", StringComparison.Ordinal), $"Disposition must preserve open FCR truth: issue {pair.Key}");
                Require(d["disposition_basis"].Contains("Application workstream remains OPEN", StringComparison.Ordinal), $"Disposition must preserve open Application truth: issue {pair.Key}");
    ...
    private static List<Dictionary<string, string>> ValidManifestRows()
    {
        ...
        {
            var wp = $"WP-{i:00}";
            rows.Add(new(StringComparer.Ordinal)
            {
                ["manifest_version"]="1", ["stage_id"]="STAGE6", ["work_package"]=wp, ["accepted_scope_label"]=ExpectedScopeByWp[wp],
                ["closure_evidence_kind"]="CANONICAL_CLOSURE_RECORD", ["closure_evidence_locator"]=$"docs/canonical-records/{wp}.txt", ["closure_evidence_sha256"]=new string('C',64),
                ["closure_decision_commit_sha"]=new string('a',40), ["accepted_technical_baseline_sha"]=new string('b',40), ["executable_evidence_sha256"]="NOT_APPLICABLE_BY_HISTORICAL_GATE",
                ["final_red_team_disposition"]="NOT_APPLICABLE_BY_HISTORICAL_GATE", ["application_compatibility_disposition"]="NOT_APPLICABLE_BY_HISTORICAL_GATE", ["historical_gate_note"]="historical"
            });
        }
        return rows;
    }
    ...
    private static void NegativeClosureSubstitution(string root, string manifestPath)
    {
        ...
        foreach(var f in new[]{"closure_evidence_locator","closure_evidence_sha256","closure_decision_commit_sha","accepted_technical_baseline_sha","executable_evidence_sha256"})
            (rows[0][f],rows[1][f])=(rows[1][f],rows[0][f]);
    ...
    { var row=Clone(Tsv.Read(manifestPath,ManifestColumns).Rows[0]); row["closure_evidence_sha256"]=new string('0',64); Throws(()=>ValidateCanonicalEvidenceForRow(root,row)); }
    ...
    private static List<Dictionary<string,string>> CensusRows()=>new()
    {
        ...
        new(StringComparer.Ordinal){["census_version"]="2",["captured_at_utc"]="2026-08-10T20:24:27Z",["issue_number"]="30",["issue_title"]="[FCR-0030] future",["status"]="NEEDS_FOUNDATION_RECONCILIATION",["waiting_on"]="FOUNDATION",["target_foundation_stage_wp"]="Stage 13",["stage6_relevance"]="NOT_STAGE6_RELEVANT",["issue_updated_at"]="2026-08-10T19:54:18Z"}
    };
    ...
    private static void NegativeExtraDisposition()=>Throws(()=>ValidateFcrRows(CensusRows(),new[]{Disp(),Disp("30","NEEDS_FOUNDATION_RECONCILIATION","FOUNDATION","Stage 13","NON_BLOCKING_FUTURE_STAGE_OR_SEPARATE_GATE","future stage")},new string('A',64)));
    ...
    private static void ApplicationAckDoesNotCloseApplicationOrFcr()
    {
        ...
        var closed=CensusRows(); closed[0]["status"]="CLOSED"; Throws(()=>ValidateFcrRows(closed,new[]{Disp(status:"CLOSED")},new string('A',64)));
        var none=CensusRows(); none[0]["waiting_on"]="NONE"; Throws(()=>ValidateFcrRows(none,new[]{Disp(waiting:"NONE")},new string('A',64)));
        Throws(()=>ValidateFcrRows(CensusRows(),new[]{Disp(basis:"Compatibility ACK received; final Application implementation verification remains pending.")},new string('A',64)));
    }

    private static void DeterministicManifestIdentity()
    {
        var text=string.Join("\n",ValidManifestRows().Select(r=>string.Join("\t",ManifestColumns.Select(c=>r[c]))));
        var a=Sha256Text(text); var b=Sha256Text(text); Require(a==b,"Manifest identity not deterministic."); Require(a!=Sha256Text(text+"x"),"Manifest mutation did not change identity.");
    }

    private static void DeterministicIntegratedClosureIdentity(string root,string manifestPath,string censusPath,string dispositionPath)
    {
        var m=Sha256GitBlob(root,"HEAD:"+NormalizeRelative(root,manifestPath));
        var c=Sha256GitBlob(root,"HEAD:"+NormalizeRelative(root,censusPath));
        var d=Sha256GitBlob(root,"HEAD:"+NormalizeRelative(root,dispositionPath));
        var a=IntegratedIdentity(m,c,d); var b=IntegratedIdentity(m,c,d); Require(a==b,"Integrated closure identity not deterministic.");
        var mm=(m[0]=='0'?"1":"0")+m[1..]; Require(a!=IntegratedIdentity(mm,c,d),"Integrated closure identity is not mutation-sensitive.");
    }

    private static string IntegratedIdentity(string m,string c,string d)=>Sha256Text($"STAGE6-WP10-INTEGRATED-CLOSURE\nMANIFEST={m}\nCENSUS={c}\nDISPOSITION={d}");

    private static void NoStage7OrAuthorityClaim()
    {
        var surface=typeof(ProgramV2).Assembly.GetTypes().Where(t=>t.IsPublic).Select(t=>t.FullName??string.Empty).ToArray();
        Require(!surface.Any(x=>x.Contains("Stage7",StringComparison.OrdinalIgnoreCase)),"WP-10 verifier exposes Stage 7 surface.");
        Require(!surface.Any(x=>x.Contains("GrantAuthority",StringComparison.OrdinalIgnoreCase)),"WP-10 verifier exposes authority-granting surface.");
    }

    private static string FindRepositoryRoot()
    {
        var d=new DirectoryInfo(Directory.GetCurrentDirectory());
        while(d is not null){if(File.Exists(Path.Combine(d.FullName,"Falcon.Foundation.ControlledProjectFoundation.slnx")))return d.FullName;d=d.Parent;}
        throw new InvalidOperationException("Repository root not found.");
    }

    private static bool GitObjectExists(string root,string spec)
    {
        var psi=GitStart(root,"cat-file","-e",spec); using var p=Process.Start(psi)??throw new InvalidOperationException("Unable to start git."); p.WaitForExit(); return p.ExitCode==0;
    }

    private static string Sha256GitBlob(string root,string spec)
    {
        var bytes=GitBlobBytes(root,spec); return Convert.ToHexString(SHA256.HashData(bytes));
    }

    private static string GitBlobText(string root,string spec)=>Encoding.UTF8.GetString(GitBlobBytes(root,spec));

    private static byte[] GitBlobBytes(string root,string spec)
    {
        var psi=GitStart(root,"cat-file","blob",spec); using var p=Process.Start(psi)??throw new InvalidOperationException("Unable to start git.");
        using var ms=new MemoryStream(); p.StandardOutput.BaseStream.CopyTo(ms); var stderr=p.StandardError.ReadToEnd(); p.WaitForExit();
        if(p.ExitCode!=0) throw new InvalidOperationException($"Unable to read canonical Git blob {spec}: {stderr.Trim()}");
        return ms.ToArray();
    }

    private static ProcessStartInfo GitStart(string root,params string[] args)
    {
    ...
    private sealed record Tsv(IReadOnlyList<Dictionary<string,string>> Rows)
    {
        ...
        {
            var lines=File.ReadAllLines(path,Encoding.UTF8).Where(x=>x.Length>0).ToArray(); Require(lines.Length>=2,$"TSV has no data rows: {path}");
            var header=lines[0].Split('\t'); Require(expectedColumns.SequenceEqual(header,StringComparer.Ordinal),$"TSV header mismatch: {path}");
            var rows=new List<Dictionary<string,string>>();
            foreach(var line in lines.Skip(1)){var fields=line.Split('\t');Require(fields.Length==header.Length,$"TSV field count mismatch: {path}");var row=new Dictionary<string,string>(StringComparer.Ordinal);for(var i=0;i<header.Length;i++)row[header[i]]=fields[i];rows.Add(row);}
            return new Tsv(rows);
        }
    }
}

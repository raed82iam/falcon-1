# Stage 6 WP-09 — Planning v0.3 — Final Candidate
...
5. exact WP-05 pressure predecessor identity/reference and temporal relation;
6. exact explicitly supplied WP-06 request/decision identities/references when present;
7. exact explicitly supplied WP-07 accepted authoritative-allocation/effective-distribution transition references when present;
8. exact explicitly supplied WP-08 projection/signal identities/references when present;
...
11. explicit integration as-of time;
...
WP-09 SHALL NOT invent a generic `latest`, `most recent`, `current event`, timeline registry or event-selection mechanism for WP-06 requests/decisions, WP-07 mutations/effective-distribution transitions, WP-08 projections/signals or any other predecessor history.

All event-like predecessor material must be supplied as an exact accepted reference/object.

WP-09 validates whether the supplied material is coherent and applicable at the explicit integration as-of point. It does not discover or choose which historical event should be considered latest.

Absence of an optional exact event reference is valid where the integrated use case does not require it.
...
- WP-04 priority/criticality must retain its exact WP-03 allocation predecessor lineage;
...
## 8. Multi-dimensional freshness and temporal-relation model

Stage 6 predecessor dimensions may advance at different governed times.

Example: an accepted WP-07 authoritative allocation mutation may create a newer accepted capacity state while the latest supplied WP-04 priority snapshot still binds an older WP-03 allocation and the supplied WP-05 pressure snapshot legitimately inherits that older WP-04/WP-03 lineage.

WP-09 SHALL NOT silently treat those older contexts as current, and SHALL NOT invalidate the newer accepted capacity merely because those derived contexts have not yet been recomputed/reobserved.

For each supplied predecessor dimension where temporal relevance matters, WP-09 shall derive generic status with semantics equivalent to:

- `CurrentAndCoherent` — exact lineage is coherent and the supplied context is valid for the integration as-of point;
- `CoherentButLagging` — exact lineage is valid but the supplied context predates a newer accepted state relevant to interpretation;
- `Unavailable` — required context is absent or cannot be safely classified;
- `Contradictory` — supplied material cannot belong to a valid governed lineage.

Implementation names may differ.

The freshness model applies where relevant to WP-04 policy/binding context, WP-05 pressure/enforcement context, optional WP-06 request/decision context, WP-07 accepted state transitions and WP-08 projections/signals.

## 9. Freshness consequences

- lagging WP-04/WP-05 context SHALL NOT be presented as current context;
- lagging WP-04/WP-05 context SHALL NOT erase or invalidate a newer exact accepted WP-07 capacity state;
- any integrated classification or consumption path requiring current priority/criticality/pressure context SHALL fail closed when that required context is lagging or unavailable;
- an exact supplied WP-06 decision remains decision truth and SHALL NOT be upgraded to applied capacity;
- accepted lower-capacity mutation remains accepted even if later policy/pressure context has not yet been recomputed/reobserved;
...
- a supplied WP-08 projection/signal whose own predecessor binding is coherent may be consumed, but WP-09 SHALL expose any relevant lagging predecessor context rather than silently upgrading it.

## 10. Mixed-state and contradiction rejection

WP-09 SHALL reject integrated material containing any of the following:

- mixed incompatible resource epochs;
- predecessor identity fork or substitution;
- a predecessor object claiming a lineage different from the exact lineage it embeds;
...
A merely older but lineage-valid predecessor is classified through the freshness model rather than automatically treated as contradiction.

## 11. Foundation-internal cross-subsystem consumption boundary

WP-09 shall provide generic read-only Foundation-internal coherence packaging so Foundation subsystems can consume one validated Stage 6 predecessor set without independently reimplementing lineage joining.
...
## 12. Existing WP-08 Application-facing consumption
...
- validate that a supplied WP-08 projection/signal is coherent with the exact supplied integrated predecessor set;
- expose integration evidence/status to Foundation-internal consumers;
- support Application compatibility verification against the existing WP-08 boundary where an active FCR requires it.
...
## 13. Integrated coherence classification

WP-09 may derive a generic classification summarizing integration health only.

Semantics shall distinguish at least:

- coherent/current;
- coherent with one or more lagging predecessor dimensions;
- unavailable;
- contradictory.

It may surface that an exact supplied accepted WP-08 signal is advisory or binding only by referencing that exact accepted WP-08 signal and its predecessor basis. It SHALL NOT recreate the signal decision.

## 14. Temporal hardening

WP-09 shall validate temporal ordering across supplied predecessor evidence and accepted state.

At minimum:

- no evidence from the future relative to the explicit integration as-of time;
...
- lagging predecessor dimensions are represented explicitly rather than silently upgraded to current truth;
...
Integrated coherence identity shall include all semantically material supplied predecessor identities/references, exact scope keys, generation/fence material, per-dimension freshness/coherence classification and explicit integration as-of time where applicable.
...
- explicitly supplied request/decision references;
- explicitly supplied WP-08 projection/signal references.

A semantically material predecessor/reference/classification change shall change integrated identity.

Reordering equivalent input collections shall not change integrated identity.

## 16. Successor-compatible verifier ownership hardening
...
## 17. Application neutrality and zero-Application validity
...
- a latest-event selector or history registry;
...
## 20. Expected implementation placement
...
- predecessor-verifier adjustments only under the exact traced rule in section 16;
...
Permanent public production names SHALL remain generic and SHALL NOT contain Stage/WP identity tokens.

## 21. Mandatory verifier coverage

### Integrated lineage

- positive coherent supplied predecessor chain through WP-08;
- zero-Application valid integrated state;
- exact resource epoch binding;
- exact allocation predecessor binding;
- exact priority/criticality predecessor lineage;
- exact pressure predecessor lineage;
- exact explicitly supplied WP-06 request/decision attribution;
- exact explicitly supplied WP-07 authoritative/effective accepted transition binding;
- exact explicitly supplied WP-08 projection/signal binding;
- deterministic integrated identity and canonical ordering;
- absence of optional event reference valid where not required.

### No-selector behavior

- no latest-request selector;
- no latest-decision selector;
- no latest-mutation selector;
- no latest-projection/signal selector;
- supplied exact reference applicability validated against explicit as-of point.

### Multi-dimensional freshness/temporal behavior

- current coherent WP-04/WP-05 context;
- coherent-but-lagging WP-04 context after newer accepted WP-07 authoritative state;
- coherent-but-lagging WP-05 pressure after newer accepted WP-07 authoritative state;
- lagging context not presented as current;
- newer accepted capacity remains accepted despite lagging derived context;
- current-context-required consumption fails closed when required context is lagging/unavailable;
...
- wrong embedded predecessor lineage rejected;
...
- integrated object preserves exact supplied predecessor references/identities;
...
- freshness-classification mutation changes identity;
...
## 23. Planning disposition

`WP09_PLANNING = PROPOSED_v0.3_FINAL_CANDIDATE`

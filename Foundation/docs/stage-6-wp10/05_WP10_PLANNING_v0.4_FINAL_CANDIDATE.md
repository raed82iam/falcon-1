...
The exact Stage 6 predecessor set is WP-01 through WP-09. Each predecessor retains its own accepted historical authority, technical baseline, evidence and closure meaning.

WP-10 may bind and verify predecessor closures. It may not reinterpret, silently reopen, or silently repair them.
...
- closure-record byte identity uses SHA-256 over exact file bytes;
...
- historical non-applicability requires the exact sentinel `NOT_APPLICABLE_BY_HISTORICAL_GATE` plus explanatory `historical_gate_note`;
...
2. `PREDECESSOR_CLOSURE_RECORD` — immutable closure evidence bound by exact path, closure commit and file-byte SHA-256;
...
- census generation must use the fresh current FCR state available to the Foundation workstream;
- every open FCR identified by the fresh sweep must be represented before relevance filtering;
- each row must classify whether the FCR has an exact Stage 6 relationship;
- the census is frozen into the exact WP-10 validation HEAD;
- the census receives a deterministic SHA-256 recorded in the final evidence package;
- the executable verifier does not call live GitHub and does not claim to prove external GitHub completeness by itself.

The fresh FCR census creation/review is a governance precondition to declaring the WP-10 candidate ready for executable validation.
...
- every Stage-6-relevant census row must have exactly one disposition row;
- no extra disposition row may exist outside the census Stage-6-relevant set;
- issue number, current status, waiting-on and target must exactly match the frozen census;
- `Waiting On: FOUNDATION` with unresolved Stage 6 obligation must be blocking;
- `Waiting On: OWNER` with unresolved Stage 6 closure-affecting decision must be blocking;
- `Waiting On: APPLICATION` is not automatically blocking when remaining work is explicitly Application-owned and the relevant Foundation WP is already accepted/closed under its historical gate;
...
Verify schema/version, exact Stage 6 identity, exact WP-01..WP-09 set, order, uniqueness, exact enums/sentinels, identity formatting, closure-record byte SHA-256, deterministic manifest identity and mutation sensitivity.

### 8.2 Closure-record binding

For each manifest row, verify declared closure record exists at the exact repository path in the validation tree and its exact file-byte SHA-256 matches the manifest.

The verifier validates supplied evidence; it does not infer or fabricate missing historical evidence.

### 8.3 Preserved closures

Verify each predecessor closure disposition is accepted/closed according to its exact historical closure record. Successor existence or WP-10 activity does not synthesize reopening.
...
- not claim that offline verification independently proves live GitHub completeness.
...
- invalid sentinel or disposition enum;
- malformed commit/baseline identity;
- malformed SHA-256;
- closure-record path missing;
- closure-record digest mismatch;
...
`WP10_PLANNING = PROPOSED_v0.4_FINAL_CANDIDATE`
